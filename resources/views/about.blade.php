...
@section('content')
      ...
      <div class="about-story-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:80px;align-items:center">
        <div>
          ...
          <div style="display:flex;gap:16px;flex-wrap:wrap">
            ...
            <a href="tel:{{ $siteSettings->phone_href }}" class="btn btn-outline">Call {{ $siteSettings->phone_display }}</a>
          </div>
        </div>
        <div style="position:relative">
          ...
          <a href="{{ $siteSettings->outside_view_url }}" target="_blank" rel="noopener noreferrer" style="position:absolute;bottom:18px;right:18px;padding:12px 18px;border-radius:999px;background:white;color:var(--primary);font-size:13px;font-weight:800;text-decoration:none;box-shadow:var(--shadow-md)">See outside ↗</a>
        </div>
      </div>
    </div>
  </section>
  ...
  <section style="padding:90px 0;background:var(--primary);color:white">
    <div class="container">
      <div class="about-facts" style="display:grid;grid-template-columns:repeat(3,1fr);gap:24px">
        <a href="{{ $siteSettings->maps_url }}" target="_blank" rel="noopener noreferrer" style="padding:32px;text-align:center;border:1px solid rgba(255,255,255,.2);border-radius:var(--radius-lg);color:white;text-decoration:none;background:rgba(255,255,255,.06)"><div style="font-size:36px;font-weight:900;color:var(--accent)">{{ $siteSettings->google_rating }} ★</div><div style="margin-top:8px">{{ $siteSettings->google_review_count }} Google reviews</div></a>
        ...
        <a href="{{ $siteSettings->directions_url }}" target="_blank" rel="noopener noreferrer" style="padding:32px;text-align:center;border:1px solid rgba(255,255,255,.2);border-radius:var(--radius-lg);color:white;text-decoration:none;background:rgba(255,255,255,.06)"><div style="font-size:34px;font-weight:900;color:var(--accent)">Bhopal</div><div style="margin-top:8px">Raisen Road · Get directions</div></a>
      </div>
    </div>
  </section>
  ...
  <section style="padding:100px 0;background:var(--bg-white)">
    <div class="container">
      <div style="text-align:center;margin-bottom:48px"><div class="section-badge" style="margin:0 auto 16px">Visit Us</div><h2 class="section-title">Find Maruti Hospital on <span style="color:var(--primary)">Raisen Road</span></h2><p class="section-subtitle" style="margin:0 auto">{{ ($siteSettings->address_line_1 . ', ' . $siteSettings->address_line_2) }}</p></div>
      <div style="border-radius:var(--radius-xl);overflow:hidden;border:1px solid var(--border-light);box-shadow:var(--shadow-md)"><iframe src="{{ $siteSettings->map_embed_url }}" width="100%" height="420" style="border:0;display:block" loading="lazy" allowfullscreen referrerpolicy="no-referrer-when-downgrade" title="Maruti Multispeciality Hospital location"></iframe></div>
      <div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap;margin-top:28px"><a href="{{ $siteSettings->directions_url }}" target="_blank" rel="noopener noreferrer" class="btn btn-primary">Get Directions</a><a href="{{ $siteSettings->outside_view_url }}" target="_blank" rel="noopener noreferrer" class="btn btn-outline">See Outside</a><a href="{{ $siteSettings->maps_url }}" target="_blank" rel="noopener noreferrer" class="btn btn-outline">Read Google Reviews</a></div>
    </div>
  </section>
